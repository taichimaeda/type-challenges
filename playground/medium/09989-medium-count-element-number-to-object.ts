...
type FlattenArray<T extends any[]> =
  T extends [infer Head, ...infer Tail]
    ? [Head] extends [never]
        ? FlattenArray<Tail>
        : Head extends any[] ? [...FlattenArray<Head>, ...FlattenArray<Tail>] : [Head, ...FlattenArray<Tail>]
    : []
type CountByArray<T extends any[], Map extends Record<PropertyKey, any[]> = {}> =
  T extends [infer Head, ...infer Tail]
    ? Head extends keyof Map
      ? CountByArray<Tail, { [P in Head]: [0, ...Map[P]] } & Omit<Map, Head>>
      : CountByArray<Tail, { [P in Head extends PropertyKey ? Head : never]: [0] } & Map>
    : Map
type MapArrayToNumber<T extends Record<PropertyKey, any[]>> = {
  [P in keyof T]: T[P]['length']
}
type CountElementNumberToObject<T extends any[]> = MapArrayToNumber<CountByArray<FlattenArray<T>>>
