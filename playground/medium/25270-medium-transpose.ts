...
type NthColumn<M extends number[][], N extends number> =
  M extends [infer Head extends number[], ...infer Tail extends number[][]]
    ? [Head[N], ...NthColumn<Tail, N>]
    : []
type Transpose<M extends number[][], Column extends any[] = []> =
  M extends [] ? [] :
    M[0]['length'] extends Column['length']
      ? []
      : [NthColumn<M, Column['length']>, ...Transpose<M, [0, ...Column]>]
