...
type SignParser<A extends string> =
  A extends `${infer Sign extends '+' | '-'}${infer Rest}` ? [Sign, ...BodyParser<Rest>] : ['', ...BodyParser<A>]
type BodyParser<A extends string> =
  A extends `${infer Body}%` ? [Body, '%'] : [A, '']
