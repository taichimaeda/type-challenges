...
type ParseUrlParams<T extends string> =
  T extends `${infer Prefix}:${infer Param}/${infer Suffix}` ? Param | ParseUrlParams<`${Prefix}${Suffix}`> :
    T extends `${infer Prefix}:${infer Param}` ? Param | ParseUrlParams<Prefix> : never
