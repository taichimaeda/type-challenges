...
type Unique<T extends any[], Seen extends any[] = []> =
  T extends [infer Head, ...infer Tail]
    ? Includes<Seen, Head> extends true ? Unique<Tail, Seen> : [Head, ...Unique<Tail, [Head, ...Seen]>]
    : []

// NOTE:
// This solution does not use `Equal` and does not work for non-literal types.
// type Unique<T extends any[], Seen = never> =
//   T extends [infer Head, ...infer Tail]
//     ? Head extends Seen ? Unique<Tail, Seen> : [Head, ...Unique<Tail, Seen | Head>]
//     : []
