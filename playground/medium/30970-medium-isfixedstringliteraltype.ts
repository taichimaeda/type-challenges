...
// https://github.com/type-challenges/type-challenges/issues/31481
type IsFixedStringLiteralType<S extends string, T = S> =
  [S] extends [never] ? false :
    S extends unknown
      ? [T] extends [S] ? IsEmbedded<T> : false
      : never
type IsEmbedded<T extends string> =
  Equal<T, string> extends true ? false :
    Equal<T, string & {}> extends true ? false :
      // For embedded types like `${number}` match `Head`
      // rather than the first character in string.
