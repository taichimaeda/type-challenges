...
type PopRight<T extends string> =
  T extends `${infer Rest} ` ? Rest : ''
type CombinationSub<T extends string, U extends string = T> =
  [T] extends [never] ? '' :
    T extends string ? `${T} ${CombinationSub<Exclude<U, T>>}` | CombinationSub<Exclude<U, T>> : never
type Combination<T extends string[]> = Exclude<PopRight<CombinationSub<ArrayToUnion<T>>>, ''>
