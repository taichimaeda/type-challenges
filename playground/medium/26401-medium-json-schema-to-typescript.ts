...
type JSONEnumSchema = unknown[]
type JSONPropertiesSchema = Record<string, JSONSchema>
type JSONItemsSchema = JSONSchema
...
type JSONSchema = {
  ...
  required?: string[]
  items?: JSONItemsSchema
}

type Compute<T> = {
  [P in keyof T]: T[P]
}
type ArrayToUnion<T extends unknown[] | undefined> =
  T extends unknown[]
    ? T extends [infer Head, ...infer Tail] ? Head | ArrayToUnion<Tail> : never
    : never

type JSONEnum2TS<T extends JSONEnumSchema | undefined> =
  T extends [infer Head, ...infer Tail] ? Head | JSONEnum2TS<Tail> : never
type JSONObject2TS<T extends JSONPropertiesSchema | undefined, Required = never> =
  T extends JSONPropertiesSchema
    ? Compute<{ [P in Exclude<keyof T, Required>]?: JSONSchema2TS<T[P]> } & { [P in Extract<keyof T, Required>]: JSONSchema2TS<T[P]> }>
    : Record<string, unknown>
type JSONArray2TS<T extends JSONItemsSchema | undefined> =
  T extends JSONItemsSchema
    ? JSONSchema2TS<T>[]
    : unknown[]
type JSONPrimitive2TS<T extends JSONSchema> =
  T['type'] extends 'string' ? string :
    T['type'] extends 'number' ? number :
      T['type'] extends 'boolean' ? boolean : never
type JSONSchema2TS<T extends JSONSchema> =
  T['enum'] extends any[] ? JSONEnum2TS<T['enum']> :
    T['type'] extends 'object' ? JSONObject2TS<T['properties'], ArrayToUnion<T['required']>> :
      T['type'] extends 'array' ? JSONArray2TS<T['items']> : JSONPrimitive2TS<T>
