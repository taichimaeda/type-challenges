...
type StringToUnion<S extends string> =
  S extends `${infer Head}${infer Tail}` ? Head | StringToUnion<Tail> : never

type AllCombinations<S extends string, T = StringToUnion<S>, U = T> =
  [T] extends [never] ? '' :
    T extends string ? `${T}${AllCombinations<S, Exclude<U, T>>}` | AllCombinations<S, Exclude<U, T>> : never
